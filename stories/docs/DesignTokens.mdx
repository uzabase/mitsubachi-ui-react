...
<Meta title="Docs/デザイントークン" />

# デザイントークン

mitsubachi-ui-react では、デザイントークンを **CSS カスタムプロパティ（CSS 変数）** として定義しています。
各トークンにはフォールバック値が設定されており、ホストアプリケーション側でカスタムプロパティを上書きすることでテーマのカスタマイズが可能です。
...
> **Note:** 以下のトークン一覧は、現在コンポーネントの実装で使用されているものをまとめたものです。Figma のデザイン定義にはここに掲載されていないトークンが存在する可能性があります。正式なトークン一覧については Figma を確認してください。
...
| `--object-regular-default`  | `rgba(0,0,0,0.84)` | 通常オブジェクト（チェックマーク等） |
